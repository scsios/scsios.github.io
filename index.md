...
2. iPhone の「設定」の中の「アクセシビリティ」の中の「音声コントロール」の中の「コマンド」の「カスタム」の中の「新規コマンドを作成」を選択し、以下の3種類のコマンドを作成する。
    ...
    あらかじめ Gemini マイクを開くショートカットを作っておき、そのショートカット名と同じ語句で作成。ここでは一例として「教えてください」とする。  
      ...
      アプリの欄は Gemini を選択。
    3. **「終わりにしてください」**  
    「質問を送信」ボタンと 「Gemini Live」ボタンが同じ位置にあるので、マイクが開いていないときに「どうぞ」と言ってしまうと、間違えて Gemini Live を起動する可能性がある。Gemini Live を Siri で終わらせることはできない。その場合でも終了させることができるようにするために、「終わりにしてください」などの語句で作成。（音声コントロールがオンである間は「ホーム画面に移動」と言っても同じ効果があるが、それを正確に言えない場合のためのカスタムコマンド。）
        1. ショートカットアプリで「ホーム画面へ移動」のショートカットを作っておく。
        2. iPhone の「設定」の中の「アプリ」の「Gemini」の「ライブアクテビティ」の中で「ライブアクティビティを許可」の項目をオフにしておく（ホーム画面に戻れば Gemini Live が終わるようにするため）。
