...
1. ショートカットアプリのオートメーションを2つ作成する。
  - オートメーションの画面で右上の＋をタップ。「アプリ」をタップ。アプリの「選択」ではGeminiを選択。「開いている」だけを選択（「閉じている」は選択しない）。「すぐに実行」を選択。次へ。「新規ショートカットを作成」（Voice Over オンのときはダブルタップ長押しでアクティベート可能）、アクションを検索で、「音声コントロールを設定」を選択。「オン」に「変更」というアクションにして完了。
  - 同様にして、Geminiが「閉じている」ときに「音声コントロールをオフに変更」するオートメーションも作成。
をオンにする。目視で操作している人は、「オーバーレイ」の項目を「なし」に設定すれば、視覚的に煩わしくない。
2. iPhone の「設定」の中の「アクセシビリティ」の中の「音声コントロール」の中の「コマンド」の「カスタム」の中の「新規コマンドを作成」を選択し、以下の3種類のコマンドを作成する。
   ...
   - **「教えてください」**  
    あらかじめ Gemini マイクを開くショートカットを作っておき、そのショートカット名と同じ語句で作成。ここでは一例として「教えてください」とする。  
      ...
      アプリの欄は Gemini を選択。
   - **「終わりにしてください」**  
    「質問を送信」ボタンと 「Gemini Live」ボタンが同じ位置にあるので、マイクが開いていないときに「どうぞ」と言ってしまうと、間違えて Gemini Live を起動する可能性がある。その場合でも終了させることができるようにするために、「終わりにしてください」などの語句で作成。  
      このコマンドを作成する前に、あらかじめ、以下の2点をやっておく：  
     ...
     2. iPhone の「設定」の中の「アプリ」の「Gemini」の「ライブアクテビティ」の中で「ライブアクティビティを許可」の項目をオフにしておく（ホーム画面に戻れば Gemini Live が終わるようにするため）。  
      作成するコマンドのアクションは「ショートカットの実行」で「ホーム画面へ移動」を選択。  
      アプリの欄は Gemini を選択。
...
- 以上の運用では Gemini 利用中だけ音声コントロールがオンになっているので、思いがけない動作をするかもしれない。
- Geminiアプリ内のテキストフィールドを開いてしまうと、「質問を送信」ボタンなどの位置がズレてしまうので、なるべく画面に触らずに利用する方が失敗しにくい。
- Voice Over を併用する場合は、 Gemini 使用中は3本指ダブルタップで「読み上げオフ」にするほうが煩わしくない。
